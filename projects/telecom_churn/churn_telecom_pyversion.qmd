...
churn_data['Churn'].value_counts(sort = True)
```
...
From the plot above, we can see how imbalance our data set is. We only have 14.49% instance of customers that churn while 85% are the customers that did not churn. Since our focus is to effectively identify customers who are most likely to churn, we need to treat this imbalance.

### Random Under-sampling

Random Under-sampling is an approach where in we reduce the percentage of the majority class for us to have a class-balanced data set.
...
On the other hand, Random Over-sampling is an approach where we add more samples to the minority class.
...
plt.clf()
ax = y_res_ros.value_counts().plot.pie(autopct = '%.2f')
...
auc = np.round(roc_auc_score(y_test,logistic_predictions),3)
...
Check performance if over or under fit

```{python}

scores_dtree = cross_val_score(clf, X_train, y_train, scoring = 'accuracy',cv = cv, n_jobs = -1)
...
auc = np.round(roc_auc_score(y_test,dtree_prediction),3)
print("AUC for our sample data is {}".format(auc))
```
